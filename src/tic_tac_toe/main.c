...
void update_ui_board(TttAppData *app)
{
    ...
    if (app->game.game_over) {
        gtk_label_set_text(GTK_LABEL(app->lbl_status), "Game Over");
    } else if (app->game.current_player == 1) {
        char status[100];
        snprintf(status, sizeof(status), "%s's Turn (X)", app->game.name1);
        gtk_label_set_text(GTK_LABEL(app->lbl_status), status);
    } else {
        char status[100];
        snprintf(status, sizeof(status), "%s's Turn (O)", app->game.name2);
        gtk_label_set_text(GTK_LABEL(app->lbl_status), status);
    }
...
GtkWidget* ttt_create_ui(void)
{
    ...
    gtk_widget_add_css_class(app->lbl_status, "subtitle");
    gtk_widget_set_margin_bottom(app->lbl_status, 15);
    gtk_widget_set_margin_top(app->lbl_status, 10);
    gtk_box_append(GTK_BOX(game_card), app->lbl_status);
