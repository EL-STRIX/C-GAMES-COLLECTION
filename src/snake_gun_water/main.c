...
static gboolean sgw_on_show_final_results(gpointer user_data) {
    ...
    char best_player[50];
    int best_score = load_top_score("sgw", best_player, sizeof(best_player));
...
static void sgw_start_new_game(SgwAppData *data) {
    ...
    sgw_update_round_display(data);
    sgw_update_score_display(data);
...
static void sgw_process_round(SgwAppData *data, int user_choice) {
    ...
    if (data->current_round < TOTAL_ROUNDS) {
        ...
        /* increment once more so sgw_update_round_display() shows calculation state in callback */
        data->current_round++;
        /* schedule final result display after 1 second */
        g_timeout_add_seconds(1, sgw_on_show_final_results, data);
    }
}
...
static GtkWidget* sgw_create_game_page(SgwAppData *data) {
...
static GtkWidget* sgw_create_result_page(SgwAppData *data) {
    GtkWidget *vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 20);
    gtk_widget_set_valign(vbox, GTK_ALIGN_CENTER);
    gtk_widget_set_halign(vbox, GTK_ALIGN_CENTER);
    gtk_widget_set_vexpand(vbox, TRUE);
...
GtkWidget* sgw_create_ui(void)
{
    ...
    GtkWidget *header = create_game_header("Snake Gun Water", FALSE);
