...
const char *css_data =
    /* --- YOUR PROVIDED CSS --- */
    "window { background-color: #cfcfcf; }"
    ...
    /* Game Status */
    ".success { color: #00c853; font-weight: bold; font-size: 14pt; }" /* Green for P1 */
    ".error { color: #d50000; font-weight: bold; font-size: 11pt; }"   /* Red for P2 */
    
    /* --- TIC TAC TOE SPECIFIC ADDITIONS (Matching your theme) --- */
    ...
    ".player-x { color: #2979ff; }" /* Matches welcome text */
    ".player-o { color: #d50000; }" /* Matches exit button */
    ;
...
static void activate(GtkApplication *app, gpointer user_data)
{
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Tic Tac Toe - Modern");
    ...
    gtk_widget_add_css_class(lbl_icon, "game-title"); // Updated class

    GtkWidget *lbl_welcome = gtk_label_new("Enter Player Details");
    gtk_widget_add_css_class(lbl_welcome, "welcome-text"); // Updated class

    // Inputs
    GtkWidget *lbl_p1 = gtk_label_new("Player 1 Name:");
    gtk_widget_set_halign(lbl_p1, GTK_ALIGN_START);
    gtk_widget_add_css_class(lbl_p1, "input-label");

    entry_p1 = gtk_entry_new();
    gtk_entry_set_placeholder_text(GTK_ENTRY(entry_p1), "Name...");
    gtk_widget_add_css_class(entry_p1, "styled-entry"); // Updated class

    GtkWidget *lbl_p2 = gtk_label_new("Player 2 Name:");
    gtk_widget_set_halign(lbl_p2, GTK_ALIGN_START);
    gtk_widget_add_css_class(lbl_p2, "input-label");

    entry_p2 = gtk_entry_new();
    gtk_entry_set_placeholder_text(GTK_ENTRY(entry_p2), "Name...");
    gtk_widget_add_css_class(entry_p2, "styled-entry"); // Updated class

    // Start Button (Using ID selector for your specific css)
    GtkWidget *btn_start = gtk_button_new_with_label("START GAME");
    gtk_widget_set_name(btn_start, "start_btn"); // Sets ID to #start_btn
    g_signal_connect(btn_start, "clicked", G_CALLBACK(on_start_clicked), NULL);

    // Tip Label
    GtkWidget *lbl_tip_start = gtk_label_new("Good luck, have fun!");
    ...
    gtk_box_append(GTK_BOX(start_card), lbl_p2);
    ...
    // Reset button setup (Reusing Start Btn style for consistency or simple button)
    GtkWidget *btn_reset = gtk_button_new_with_label("Restart Round");
    ...
    // Reuse start button style by ID
    gtk_widget_set_name(btn_rematch, "start_btn"); 
    g_signal_connect(btn_rematch, "clicked", G_CALLBACK(on_play_again_clicked), NULL);

    GtkWidget *btn_end = gtk_button_new_with_label("Exit Game");
    gtk_widget_add_css_class(btn_end, "btn-exit"); // Updated class
...
int main(int argc, char **argv)
{
    ...
    app = gtk_application_new("org.sujay.tictactoe", G_APPLICATION_DEFAULT_FLAGS);
